...
// SVG: Scharnierachse bei cx=150, cy=100
// rudderGroup dreht sich um diesen Punkt

function updateRudderGraphic(angleDeg) {
    // Ruder drehen (CSS transform über SVG)
    rudderGroup.setAttribute('transform', `rotate(${angleDeg}, 150, 100)`);

    // Winkel-Label
    angleLabel.textContent = angleDeg.toFixed(1) + '°';

    // Winkel-Bogen zeichnen (kleiner Kreisbogen zur Visualisierung)
    if (Math.abs(angleDeg) > 0.5) {
        const r       = 35;
        const cx      = 150, cy = 100;
        const startRad = 0;                          // 0° = Ruder neutral (rechts)
        ...
        // Label-Position leicht versetzt
        ...
        anglePath.setAttribute('d', '');
        angleLabel.setAttribute('x', 175);
        angleLabel.setAttribute('y', 78);
    }
}
